...
type RestOf<T extends IInvoice | NewInvoice> = Omit<T, 'issuedAt' | 'dueAt'> & {
...
export class InvoiceService {
  ...
  protected convertDateFromClient<T extends IInvoice | NewInvoice | PartialUpdateInvoice>(invoice: T): RestOf<T> {
    return {
      ...invoice,
      issuedAt: invoice.issuedAt?.toJSON() ?? null,
      dueAt: invoice.dueAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInvoice: RestInvoice): IInvoice {
    return {
      ...restInvoice,
      issuedAt: restInvoice.issuedAt ? dayjs(restInvoice.issuedAt) : undefined,
      dueAt: restInvoice.dueAt ? dayjs(restInvoice.dueAt) : undefined,
    };
  }
