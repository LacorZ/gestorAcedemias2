...
public class TutorRepositoryWithBagRelationshipsImpl implements TutorRepositoryWithBagRelationships {
    ...
    public Optional<Tutor> fetchBagRelationships(Optional<Tutor> tutor) {
        return tutor.map(this::fetchCursos);
    }
    ...
    public List<Tutor> fetchBagRelationships(List<Tutor> tutors) {
        return Optional.of(tutors).map(this::fetchCursos).orElse(Collections.emptyList());
    }

    Tutor fetchCursos(Tutor result) {
        return entityManager
            .createQuery("select tutor from Tutor tutor left join fetch tutor.cursos where tutor is :tutor", Tutor.class)
            .setParameter("tutor", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Tutor> fetchCursos(List<Tutor> tutors) {
        ...
        List<Tutor> result = entityManager
            .createQuery("select distinct tutor from Tutor tutor left join fetch tutor.cursos where tutor in :tutors", Tutor.class)
