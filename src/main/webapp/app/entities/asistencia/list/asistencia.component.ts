...
import { EntityArrayResponseType, AsistenciaService } from '../service/asistencia.service';
...
export class AsistenciaComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAsistencia): number => this.asistenciaService.getAsistenciaIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.asistencias = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IAsistencia[] | null): IAsistencia[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.asistenciaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
