...
type RestOf<T extends IFactura | NewFactura> = Omit<T, 'fechaFactura'> & {
...
export class FacturaService {
  ...
  protected convertDateFromClient<T extends IFactura | NewFactura | PartialUpdateFactura>(factura: T): RestOf<T> {
    return {
      ...factura,
      fechaFactura: factura.fechaFactura?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restFactura: RestFactura): IFactura {
    return {
      ...restFactura,
      fechaFactura: restFactura.fechaFactura ? dayjs(restFactura.fechaFactura) : undefined,
    };
  }
