...
type RestOf<T extends IPago | NewPago> = Omit<T, 'fechaPago'> & {
...
export class PagoService {
  ...
  protected convertDateFromClient<T extends IPago | NewPago | PartialUpdatePago>(pago: T): RestOf<T> {
    return {
      ...pago,
      fechaPago: pago.fechaPago?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPago: RestPago): IPago {
    return {
      ...restPago,
      fechaPago: restPago.fechaPago ? dayjs(restPago.fechaPago) : undefined,
    };
  }
